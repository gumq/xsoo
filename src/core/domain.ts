...
export interface Statistics { generatedAt: string; drawCount: number; frequency: NumberMetric[]; specialFrequency: SpecialMetric[]; specialByHour: Record<string, SpecialMetric[]>; specialAssociations: Record<string, SpecialAssociation[]>; specialTransitions: Record<string, SpecialAssociation[]>; specialBacktest: SpecialBacktest; comboBacktests: ComboBacktest[]; recommendations: Recommendation[]; pairs: Array<{ key: string; count: number }>; distributions: { oddEven: Record<string, number>; highLow: Record<string, number>; sums: number[]; }; rollingMeans: number[]; }
