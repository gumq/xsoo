...
// The dashboard needs the full history for the user-entered ticket checker.
const recentSevenDays = (records: EventRecord[]) => records;
...
export default async () => {
  ...
  try {
    ...
    return json({ sourceStatus: 'live', checkedAt: new Date().toISOString(), metadata, history: records, cache: { generatedAt: new Date().toISOString(), statistics: new StatisticsService(defaultRules).build(records) } });
  } catch {
